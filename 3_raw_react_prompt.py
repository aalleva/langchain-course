...
# --- Tools (plain functions with @traceable for LangSmith tracing) ---
# CHANGE 2: Drop @tool decorator — tools are now plain functions called by our loop.
...
# CHANGE 4: Drop bind_tools(). The LLM has no idea it's an agent —
# all agency comes from the prompt above and our regex parsing below.

@traceable(name="OpenAI Chat", run_type="llm")
def openai_chat_traced(llm, prompt_text: str) -> str:
    """Call OpenAI via LangChain with a stop sequence to prevent self-generated Observations."""
    llm_with_stop = llm.bind(stop=["\nObservation"])
    response = llm_with_stop.invoke([HumanMessage(content=prompt_text)])
...
def run_agent(question: str) -> str:
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        output = openai_chat_traced(llm, full_prompt)
        print(f"LLM Output:\n{output}")
...
def main() -> None:
    print("Hello Raw ReAct Prompt Agent (OpenAI)!")
    print()
